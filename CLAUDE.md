...
- `src/main/everythingSdk.ts` — lazy-loaded on win32 only (top comment explains why). Spawns voidtools **es.exe** (the official Everything CLI) instead of the C SDK; es.exe talks to the running Everything client over window-message IPC and writes UTF-8 TSV to a temp file, so non-ASCII paths survive. es.exe discovery order: custom path → next to `Everything.exe` (registry App Paths / Uninstall InstallLocation / Run key / service ImagePath, read via PowerShell for Unicode-safe installs like `D:\软件\Everything`) → default install dirs → bundled `resources/everything/es.exe`. Two es.exe runs per search: `-export-tsv` (results, capped) + `-get-result-count` (un-capped total). **Auto-remediation**: when a probe reports the DB isn't reachable (exit code 8 = IPC not found), `triggerAutoStart()` spawns `Everything.exe -startup` and polls until es.exe answers (throttled 60s); negative availability is not cached so the next search re-probes. Keeps a 300-entry debug ring buffer + last query/error state, exposed via `getDebugInfo()`. Searches are serialised through a queue; checkAvailability has a 30s positive cache.
...
- Diagnostics UI: `src/renderer/components/dialogs/EverythingDebugDialog.tsx` — opened from Settings → General next to the Everything toggle. Polls `getEverythingDebugInfo` every 1s: installed/running/es.exe/DB status chips, exe/es.exe paths, last query/error, test-search box, live log, start/install buttons.
- There are no native (FFI) dependencies — the old koffi + Everything64.dll approach was removed. The packaged app ships `resources/everything/es.exe` (64-bit, freeware) so it works even when the Everything install lacks ES.
